...
$(function () {
	 ...
	 * Persistant Data
	************************************************************************************/
	var tournamentPersistantDataReplicant = nodecg.Replicant("tournamentPersistantData");
    tournamentPersistantDataReplicant.on("change", function (newValue, oldValue) {
	...
	************************************************************************************/
    $smashggUpdateButton.click(function () {
        if ($smashggTournamentName.val() != '') { // Makes HTTP call to try and download new tournament
            nodecg.sendMessage("smashgg-gettournament", $smashggTournamentName.val(), function (error, response, body) {
                ...
                var tournamentContainer = tournamentPersistantDataReplicant.value;
                ...
                tournamentPersistantDataReplicant.value = tournamentContainer;
    ...
    $smashggPhaseGroupButton.click(function () {
        nodecg.sendMessage("smashgg-getphasegroup", $smashggPhaseGroupsDropdown.val(), function (error, response, body) {
            var b = JSON.parse(body);
            //console.log("PhaseGroup: " + $smashggPhaseGroupsDropdown.val() + " - ", b);
			
            var tableItems = "<tr><th>Match</th><th>P1</th><th>P2</th><th>C</th></tr>";
            ...
            sets.forEach(function (item) {
                ...
                if (player1 && player2) {
                    var control = "<input type='button' id='sendmatch-" + item.id + "'></input><input type='checkbox'></input>";
                    var newItem = "<tr><td>" + item.fullRoundText + "</td><td>" + player1.gamerTag + "</td><td>" + player2.gamerTag + "</td><td>" + control + "</td></tr>";
                    tableItems += newItem;
                }
            });
            ...
            sets.forEach(function(item) {
                ...
                if (typeof e !== 'undefined' && e !== null) {
                    ...
                    e.click(function() {
                        ...
                        nodecg.sendMessage("smashgg-sendplayerdata", { p1: player1, p2: player2 });
